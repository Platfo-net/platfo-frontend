...
class Node {
  getAllNodes = (params, chatflow_id) =>
    $axios.get(`api/v1/node/all/${chatflow_id}`, { params });

  postcreateNode = (data) => $axios.post(`api/v1/node`, data);
  postCreateFullNode = (data) =>
    $axios.post(`api/v1/node/full`, data);

  postMessageWidget = (data, node_id) =>
    $axios.post(`api/v1/node/${node_id}/message`, data);
  ...
  getConnectWidget = (params, node_id, from_id) =>
    $axios.get(`api/v1/node/connect/${node_id}/${from_id}`, {
      params,
    });

  getNextMove = (params, from_id) =>
    $axios.get(`api/v1/node/${from_id}/next`, { params });
